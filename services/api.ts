...
export const api = {
  ...
  adminLogin: async (email: string, password: string): Promise<User> => {
    ...
    const user = users.find(u => u.email.toLowerCase() === email.toLowerCase() && u.role === UserRole.ADMIN);
  ...
  createEmployee: async(employeeData: Omit<Employee, 'id' | 'dealer_id' | 'status'>): Promise<Employee> => {
    ...
    if (!currentUser || !currentUser.dealerId) throw new Error("Not authorized");
    if (employees.some(e => e.aadhar === employeeData.aadhar)) {
        throw new Error("An employee with this Aadhar number already exists.");
    }
